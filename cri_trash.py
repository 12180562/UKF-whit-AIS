...
class CRI:
    def __init__(self, L, B, Xo, Yo, Xt, Yt, Co, Ct, Vo, Vt, ship_scale):
        ...
        self.ratio = 1820/(self.ship_scale)  # it is the ratio that is considered the scale effect and conversion of units

        # All the length units for CRI is nm. so it have to be conveted to m unit.
        # Also, All the length have to be modified because of the scale.
        # ratio is the parameter for converting.
        # ex) if the scale of ship is 10, the ratio is 1820/10
    ...
    def CRI(self):
        ...
        return result,  0.4 * self.UDCPA(),0.367 * self.UTCPA(), 0.133 * self.UD(), 0.067 * self.UB(), 0.033 * self.UK()
