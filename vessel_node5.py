...
class data_inNout:
    ...
    def __init__(self):
        ...
        self.WP_pub = rospy.Publisher('/vessel5_info', col, queue_size=10)
        self.cri_pub = rospy.Publisher('/cri5_info', cri_info, queue_size=10)
...
def main():  
    ...
    VO_operate = rospy.get_param("shipInfo_all/ship5_info/include_inha_modules")
    ...
    # 자선의 정보
    OS_scale = rospy.get_param("shipInfo_all/ship5_info/ship_scale")
    target_speed = rospy.get_param("shipInfo_all/ship5_info/target_speed")  * 0.5144 / sqrt(OS_scale)
    ship_L = rospy.get_param("shipInfo_all/ship5_info/ship_L") ## 향후 이부분은, 1) ship domain과 2) AIS data의 선박의 길이 부분으로 나누어 고려 및 받아야 함!
    ...
    random_value = 0
    random_heading = rospy.get_param("shipInfo_all/ship5_info/random_heading")
