...
class ShipSimulation(Controller): # `Controller()`       # Speed, Steering, Heading 제어기를 포함하고 있어서 상속해줌
    """시뮬레이션을 위한 클래스"""
    def __init__(self, initial_X, initial_Y, initial_velocity, initial_Heading_deg, initial_delta_deg, ship_scale, dt):
        '''시뮬레이션을 위한 클래스
    ...
    def moving_ships(self, target_heading, target_spd):
        ...
        KASS_mmg = KASS_MMG(self.ship_scale)           # MMG 조종운동방정식을 활용하여, 자선/타선의 동역학적 특성을 반영함
        ...
        Realtime_U = sqrt(uBody**2 + vBody**2)    

        # NOTE: It changes the `self`
        n = self.speed_controller(target_spd, Realtime_U, self.ship_scale)

        ##########  `t`에서의 자선의 속도와 타각을 바탕으로 `t+1`에서의 자선의 가속도 계산 ##########
        velocity_matrix = np.array([[uBody], [vBody], [r_rad]])         # Pack the ship velocity components
        
        acceleration_matrix = KASS_mmg.uvrdot(uBody, vBody, r_rad, Realtime_U, delta_rad, n)
