...
class Controller:
    ...
    def __init__(self):
        mmg = KASS_MMG(ship_scale=rospy.get_param('OS_info/ship_scale'))
    ...
    def speed_controller(self, target_spd, Realtime_U, ship_scale):
        """ 속도 제어기라기 보다는, `desired_speed`에 대응하는 모형선의 자항점을 추정하여, 저항에 따른 속도 감속에 대응

        Returns:
            n : 목표 속도를 내기위한 자항점
        """ 
        KASS_mmg = KASS_MMG(ship_scale)  # <- `KASS_MMG()`의 self 변수로 KASS 선형의 미계수 값들이 저장되어 있어 활용하기 위해 호출!
        ...
        error_speed = target_spd - Realtime_U
        # print("error : ", error_speed)          

        if abs(error_speed) > abs(0.01 * target_spd) :
