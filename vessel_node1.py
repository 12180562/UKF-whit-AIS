...
def main():  
    ...
    VO_operate = rospy.get_param("shipInfo_all/OS_info/include_inha_modules")
    ...
    OS_ID = rospy.get_param("shipInfo_all/OS_info/ship_ID")
    TS_ID = []
    desired_spd_list = []
    pub_collision_cone = [] 
    V_opt = []
    ...
    # 자선의 정보
    OS_scale = rospy.get_param("shipInfo_all/OS_info/ship_scale")
    target_speed = rospy.get_param("shipInfo_all/OS_info/target_speed")  * 0.5144 / sqrt(OS_scale)
    ship_L = rospy.get_param("shipInfo_all/OS_info/ship_L") ## 향후 이부분은, 1) ship domain과 2) AIS data의 선박의 길이 부분으로 나누어 고려 및 받아야 함!
    ...
    while not rospy.is_shutdown():
        Local_PP = VO_module()
        radius_scale = rospy.get_param("shipInfo_all/OS_info/ship_domain_radius")
